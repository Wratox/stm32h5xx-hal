...
// Note: pin data is taken from stm32h573 and stm32h523 datasheets
            ...
            $(
                ...
                impl Spi<$SPIX, Enabled, $TY> {
                        ...
                        CONFIG: Into<Config>,
                    {
                        ...
                        spi.cr2().write(|w| w.tsize().set(matches!(config.hardware_cs.mode, HardwareCSMode::FrameTransaction) as u16));
                ...
                impl embedded_hal::spi::ErrorType for Spi<$SPIX, Enabled, $TY> {
                    type Error = Error;
                }
                impl embedded_hal::spi::SpiBus<$TY> for Spi<$SPIX, Enabled, $TY> {
	    ...
	    $(
            impl<WORD> Spi<$SPIX, Enabled, WORD> {
                ...
                pub fn kernel_clk(clocks: &CoreClocks) -> Option<Hertz> {
                    ...
                    match ccipr3.spi123sel().variant() {
                        Some(ccipr3::SPI123SEL::Pll1Q) => clocks.pll1_q_ck(),
                        Some(ccipr3::SPI123SEL::Pll2P) => clocks.pll2_p_ck(),
                        Some(ccipr3::SPI123SEL::Pll3P) => clocks.pll3_p_ck(),
                        // Need a method of specifying pin clock
                        Some(ccipr3::SPI123SEL::I2sCkin) => unimplemented!(),
                        Some(ccipr3::SPI123SEL::Per) => clocks.per_ck(),
                        _ => unreachable!(),
                    }
                }
                /// Returns the frequency of the current kernel clock
...
// TODO: Do this for spi5sel, note that spi5sel uses pclk3 instead of pclk2
macro_rules! spi46sel {
	($($SPIX:ident,)+) => {
	    $(
            impl<WORD> Spi<$SPIX, Enabled, WORD> {
                /// Returns the frequency of the current kernel clock
                ...
                pub fn kernel_clk(clocks: &CoreClocks) -> Option<Hertz> {
                    ...
                    match ccipr3.spi4sel().variant() {
                        Some(ccipr3::SPI456SEL::Apb) => Some(clocks.pclk2()),
                        Some(ccipr3::SPI456SEL::Pll2Q) => clocks.pll2_q_ck(),
                        Some(ccipr3::SPI456SEL::Pll3Q) => clocks.pll3_q_ck(),
                        Some(ccipr3::SPI456SEL::HsiKer) => clocks.hsi_ck(),
                        Some(ccipr3::SPI456SEL::CsiKer) => clocks.csi_ck(),
                        Some(ccipr3::SPI456SEL::Hse) => clocks.hse_ck(),
                        _ => unreachable!(),
                    }
                }
                /// Returns the frequency of the current kernel clock
	    ...
	    $(
            impl<WORD> Spi<$SPIX, Enabled, WORD> {
                ...
                pub fn kernel_clk(clocks: &CoreClocks) -> Option<Hertz> {
                    ...
                    match ccipr3.spi6sel().variant() {
                        Some(ccipr3::SPI6SEL::RccPclk4) => Some(clocks.pclk4()),
                        Some(ccipr3::SPI6SEL::Pll2Q) => clocks.pll2_q_ck(),
                        Some(ccipr3::SPI6SEL::Pll3Q) => clocks.pll3_q_ck(),
                        Some(ccipr3::SPI6SEL::HsiKer) => clocks.hsi_ck(),
                        Some(ccipr3::SPI6SEL::CsiKer) => clocks.csi_ck(),
                        Some(ccipr3::SPI6SEL::Hse) => clocks.hse_ck(),
                        _ => unreachable!(),
                    }
                }
                /// Returns the frequency of the current kernel clock
